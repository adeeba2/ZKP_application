import { init_web3, getOwnedItemsFromEvent, customerPartListManager, customerProductListManager, addItemToList } from "./utils.js"

window.onload = async function () {

    var x = await init_web3()

    //First, get all the parts and products that belong to this customer
    getOwnedItemsFromEvent(window.accounts[0], 'TransferPartOwnership').then((parts) => {
        console.log("part Events")
        console.log(parts)
        for (var i = 0; i < parts.length; i++) {
            addItemToList(parts[i], "parts-history", customerPartListManager)
        }
    })
    ...
    getOwnedItemsFromEvent(window.accounts[0], 'TransferProductOwnership').then((products) => {
        ...
        for (var i = 0; i < products.length; i++) {
            addItemToList(products[i], "car-history", customerProductListManager)
        }
    })
